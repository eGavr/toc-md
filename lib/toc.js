...
module.exports = function (source, options) {
    ...
    for (var i = 0; i < tokens.length; i++) {
        ...
        var tocElem = '* <a href="#' + href + '">' + header.text.replace(/\\/g, '\\\\') + '</a>' + lineBreak,
        ...
        toc.headers.push({
            index: headerIndex,
            href: ['', '<!-- TOC ELEMENT -->', '<a name="' + href + '"></a>'].join(lineBreak)
        });

        parsingIndex = headerIndex + matchedHeader[0].length;
    }

    toc.data += lineBreak + '<!-- TOC END -->';
